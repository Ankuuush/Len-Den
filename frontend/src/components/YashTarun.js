import React from 'react'
import './YashTarun.scss'
import { useNavigate } from "react-router-dom";
const YashTarun = () => {
  const navigate=useNavigate()
  const alertMessage=()=>{
    alert("Offer has been recorded")
  }
  return (
    <>
    <button type="button" class="btn btn-dark" onClick={()=>navigate("/")}>Log Out</button>
   
      <div className="card">
          
       <div className="avatar-holder">
         <img
            src="data:image/png;base64,"
            alt="User Logo"
          />
        </div>
        <div className='details'>
            <ul>
                <li>Purpose: Debt consolidation</li>
                <br />
                <li>Guarantor(Documents if any)</li>
                <br />
                <li>Guarantor Details
                    <ul>
                        <li>
                            Name : Yash Kagi
                        </li>
                        <li>
                            Address : Building No. 1696,Ground Floor,Kompally,Hyderabad
                        </li>
                        <li>
                            Contact No : 9959552541
                        </li> 
                        <li>
                            PAN No : AIQYP2356G
                        </li>
                        <li>
                            Email : yashyoyo@gmail.com
                        </li>                      
                    </ul>
                </li>
                <br />
                <li>
                    Consequences: Open to Legal Action
                </li>
            </ul>
        </div>
      </div>
      <button type="button" class="ctn btn-dark" onClick={alertMessage}>Text</button>
    </>
  )
}

export default YashTarun