import React from "react";
import './User3.scss'
import { useNavigate } from "react-router-dom";
const User3 = () => {
  const navigate=useNavigate()
  const alertMessage=()=>{
    alert("Offer has been recorded")
  }
  return (
    <>
    <button type="button" class="btn btn-dark" onClick={()=>navigate("/")}>Log Out</button>
      <div className="card">
        <div className="avatar-holder">
          <img
            src="data:image/png;base64,"
            alt="User Logo"
          />
        </div>
        <div className="details">
          <ul>
            <li>Purpose: Vehicle financing</li>
            <br />
            <li>Guarantor(Documents if any)</li>
            <br />
            <li>
              Guarantor Details
              <ul>
                <li>Name : Sumit Pandey</li>
                <li>
                  Address : House No. 126,4th Main Road,Lucknow,Utter Pradesh
                </li>
                <li>Contact No : 8998382541</li>
                <li>PAN No : BIUDP26120M</li>
                <li>Email : sumit.pan09@gmail.com</li>
              </ul>
            </li>
            <br />
            <li>Consequences: Open to legal action</li>
          </ul>
        </div>
      </div>
      <button type="button" class="ctn btn-dark" onClick={alertMessage}>Text</button>
    </>
  );
};

export default User3;
