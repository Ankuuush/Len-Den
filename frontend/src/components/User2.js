import React from "react";
import './User2.scss'
import { useNavigate } from "react-router-dom";
const User2 = () => {
  const navigate=useNavigate()
  const alertMessage=()=>{
    alert("Offer has been recorded")
  }
  return (
    <>
    <button type="button" class="btn btn-dark" onClick={()=>navigate("/")}>Log Out</button>
      <div className="card">
        <div className="avatar-holder">
          <img
            src="data:image/png;base64,"
            alt="User Logo"
          />
        </div>
        <div className="details">
          <ul>
            <li>Purpose: Home remodeling</li>
            <br />
            <li>Guarantor(Documents if any)</li>
            <br />
            <li>
              Guarantor Details
              <ul>
                <li>Name : Varun Patil</li>
                <li>
                  Address : House No. 16,1st Floor,Banshankari,Bangalore
                </li>
                <li>Contact No : 9952382541</li>
                <li>PAN No : AIDRP2956K</li>
                <li>Email : varun019@gmail.com</li>
              </ul>
            </li>
            <br />
            <li>Consequences: Open to delisting of property</li>
          </ul>
        </div>
      </div>
      <button type="button" class="ctn btn-dark" onClick={alertMessage}>Text</button>
    </>
  );
};

export default User2;
